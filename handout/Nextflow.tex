...
\begin{itemize}
  ...
  \item \texttt{params.universe = `World'} sets the default value of the command-line accessible option
...
This could be done via configuration files (more about that later) or in the process definition. 
However, for our simple use-case it should suffice to use the environmental variable.
...
The messages printed to the terminal should now include the name of the compute node on which a given task is executed.
...
We have already covered a number of limitation of that and similar approaches, 
so without further delay we will cover steps to re-implementing that workflow 
in Nextflow. 
...
We will walk you through the steps of reimplementing the first few steps of the above script 
...
%While developing, you could use the \texttt{.view()} operator for a sneak-peak 
%at what items are emitted by a given channel.
%
%\begin{lstlisting}
%Channel.fromPath('data/references/reference.fasta.gz').view()
%\end{lstlisting}

To be able to access the channel, you normally need to assign it to a variable.

\begin{lstlisting}
referencesChannel = Channel.fromPath('data/references/reference.fasta.gz') 
\end{lstlisting}

But since we often chain multiple operations, for example \\
\texttt{oneChannel.filter(it.size > n).combine(anotherChannel).collect()},\\ 
...
we also need to define the outputs for the process. The expected output files are:
...
To declare these as outputs we could 
...
We could be more explicit and declare that outputs are all (non-input)
...
and than parsing the prefix out of the file name(s), 
we opt to output our reference file name alongside the generated index file.
To do that in a single emission we wrap these in a a \emph{set} 
...
\subsection{Running the workflow (stub)}
...
Unless all the software required by the pipeline (in this case - just \texttt{bwa}) is available on the \texttt{\$PATH},
...
\begin{answer}
The cause was likely ``\texttt{.command.sh: line 2: bwa: command not found}''. Example:

\begin{lstlisting}
[68/31190c] Submitted process > bwa_index (1)
ERROR ~ Error executing process > 'bwa_index (1)'

Caused by:
  Process `bwa_index (1)` terminated with an error exit status (127)

Command executed:

  bwa index -a bwtsw reference.fasta.gz
...
Work dir:
  /shared/training001/nextflow/example_workflow/work/68/31190cc4a8d0785dba721a3caebc46
...
\begin{itemize}
 ...
 \item  Restrict how many items are emitted through a \emph{channel}
% \item  
% \item  
\end{itemize}

Paste and save the following into a new file \texttt{getpairs.nf}
\begin{lstlisting}
// Channel reading our FASTQ files
Channel.fromFilePairs("data/**_R{1,2}.fastq.gz")
  .set { readPairsForQcChannel }

// sneak-peak at what flows through our channel
readPairsForQcChannel.view()
\end{lstlisting}
...
\begin{steps}
\begin{lstlisting}
nextflow run main.nf getpairs.nf
\end{lstlisting}
\end{steps}

You will see a long list of file pairs. One way to restrict the number of input files would be 

Initially, we may want to restrict the number of file pairs being processed 
e.g. by pointing to a specific file pair only,
...
or by using one of NF operators to limit the number of items emitted by the channel.
Either \textbf{one} of \texttt{.take(1)}, \texttt{.first()} or \texttt{.last()} would suffice.  
...
// sneak-peak at what flows through our channel
readsForQcChannel.view()
\end{lstlisting}

Now that we have our input channel ready and emitting, lets define a process
to implement the original bash snippet.
...
allocation of resources as we will end up with $2 \times$ the number of tasks,
...
\begin{itemize}
 \item 
As you might have guessed, \texttt{\$\{task.cpus\}} is the number of logical cores assigned to a given process. This can be set using the \texttt{cpus} directive above the input block. The default is \texttt{cpus 1}.
...
This time we have to process the reads as pairs so using \texttt{Channel.fromPath()} 
...
[
  Gladius, 
  [
    /path/data/raw_reads/Gladius_R1.fastq.gz, 
    /path/data/raw_reads/Gladius_R2.fastq.gz
  ]
]
\end{lstlisting}
...
We now have to input channels, but how do we handle multiple input channels? 
If we just declare both separately 
